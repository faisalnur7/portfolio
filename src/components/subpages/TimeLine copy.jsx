...
const Timeline = () => {
    ...
    return (
        ...
        {timelineData.map((item, index) => (
            <li key={index}>
            {item.align === "end" && <hr />}
            <div className="timeline-middle p-5">
                <svg
            ...
            <div
                className={`timeline-${item.align} mb-10 bg-blue-950 p-8 rounded-xl shadow-md border-8 border-gray-300 ${
                item.align === "end" ? "md:text-end" : ""
                }`}>
                <Bounce>
                    <time className="font-mono italic text-white">{item.year}</time>
                    <div className="pb-2 text-lg font-black text-white">{item.title}</div>
